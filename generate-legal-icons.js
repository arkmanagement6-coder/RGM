...
let baseHtml = '';
try {
    baseHtml = fs.readFileSync('about.html', 'utf8');
} catch (e) {
    console.error("Could not read about.html");
    process.exit(1);
}

// Extract head, header, footer from baseHtml
const headMatch = baseHtml.match(/<head>[\\s\\S]*?<\\/head>/);
const headerMatch = baseHtml.match(/<header class="header scrolled">[\\s\\S]*?<\\/header>/);
const footerMatch = baseHtml.match(/<footer class="footer">[\\s\\S]*?<\\/footer>/);

if (!headMatch || !headerMatch || !footerMatch) {
    console.error("Missing standard layout in about.html");
    process.exit(1);
}

let templateHead = headMatch[0];
let templateHeader = headerMatch[0];
...
for (const [filename, pageData] of Object.entries(pages)) {
    fs.writeFileSync(filename, template(pageData.title, pageData.content), 'utf8');
